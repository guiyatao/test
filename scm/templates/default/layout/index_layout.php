...
//固定层移动
$(function(){
    <?php if($_COOKIE['pending_matters'] == true) {?>
        $.cookie('commonPendingMatters', 0, {expires : -1});ajax_form('pending_matters', '待处理事项', '<?php echo SCM_SITE_URL?>/index.php?act=common&op=pending_matters', '880');
    <?php }  setcookie('pending_matters',false);?>
	...
	function ajaxFileUpload()
	{
		...
		(
			{
				url:'<?php echo SCM_SITE_URL?>/index.php?act=common&op=pic_upload&type=admin_avatar&form_submit=ok&uploadpath=<?php echo ATTACH_ADMIN_AVATAR;?>',
				secureuri:false,
				fileElementId:'_pic',
				dataType: 'json',
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo SCM_SITE_URL?>/index.php?act=common&op=pic_cut&type=admin_avatar&x=100&y=100&resize=1&ratio=1&url='+data.url,690);
					}else{
						alert(data.msg);
					}
...
<div class="admincp-header">
  ...
  <div class="admincp-header-r">
    ...
    <ul class="operate nc-row">
      <li style="display: none !important;" nctype="pending_matters"><a class="toast show-option" href="javascript:void(0);" onclick="$.cookie('commonPendingMatters', 0, {expires : -1});ajax_form('pending_matters', '待处理事项', '<?php echo urlSCM('common', 'pending_matters');?>', '880');" title="查看待处理事项">&nbsp;<em></em></a></li>
...
<div class="admincp-container unfold">
  <div class="admincp-container-left">
    ...
    <div class="about" title="关于共铸商城 B2B2C"><i class="fa fa-copyright"></i><span>gongzhuying.com</span></div>
  </div>
