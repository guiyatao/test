...
class SCMControl{
    ...
    protected final function systemLogin() {
        ...
        if (!key_exists('gid',(array)$user) || !isset($user['sp']) || (empty($user['name']) || empty($user['id']))){
            @header('Location: '. SCM_SITE_URL .'/index.php?act=login&op=login');exit;
        }else {
    ...
    protected final function checkPermission($link_nav = null) {
        ...
        if (!defined('MODULE_NAME') || in_array($act, array('show_pics'))) return true;//modules目录外的不需要验证
...
//          if ($this->admin_info['sp'] == 1) return $menu;
    ...
    protected final function getMenu() {
        if (empty($this->menu)) {
            $this->menu  = rkcache('scm_menu', true);
        }
        return $this->menu;
    }
    ...
    protected final function scmLog($lang = '', $state = 1, $order_sn='',$admin_name = '', $admin_id = 0) {
//        if (!C('sys_log') || !is_string($lang)) return true;
        if ($admin_name == ''){
            $admin = unserialize(decrypt(cookie('sys_key'),MD5_KEY));
            $admin_name = $admin['name'];
        ...
        return SCMModel('gzkj_scm_log')->addScm_log($data);
    }
    /**
     * 输出JSON
     *
