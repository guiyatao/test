...
class LoginControl extends SCMControl {
        ...
        if ($result){
            ...
            if ($error != '') {
                ...
                if(is_array($admin_info) and !empty($admin_info)) {
                    ...
                    $user = $model_user->getUserInfo($admin_info['admin_id']);
                    switch($user['user_type']){
                        ...
                        case 2:
                            ...
                            //查出预警信息
                            $warn = $model_stock->getWarnInfo($user['supp_clie_id']);
                            if($warn['stockout_count'] > 0 || $warn['date_warn_count'] > 0 || $warn['unsalable_count'] > 0){
                                setcookie('pending_matters',true);
                            }
                            break;
                        case 3:
                            //与当前供应商合作的终端店有预警信息时开启开关
                            $model_supplier = SCMModel('supplier_account');
                            $model_supplier_client = SCMModel('supplier_client');
                            $supplier = $model_supplier->getSupplier(array('admin.admin_id'=>$admin_info['admin_id']));
                            $warn = $model_supplier_client->get_pending_matters($supplier['supp_id']);
                            if($warn == true){
                                setcookie('pending_matters',true);
                            }
                            break;
                        default:
                         $model_supp_stock = SCMModel('gzkj_supp_stock');
                            $warn = $model_supp_stock->get_pending_matters();
                            if($warn == true){
                                setcookie('pending_matters',true);
                            }
                            break;
