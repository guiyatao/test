...
class scm_client_orderModel extends Model {
    ...
    public function getOrderInfo($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '') {
        $order_info = $this->table('scm_client_order')->field($fields)->where($condition)->find();
        if (in_array('order_goods',$extend)) {
            //取商品列表
            $order_goods_list = $this->getOrderGoodsList(array('order_id'=>$condition['id']));
     ...
     * @return array
     */
    public function getOrderGoodsInfoByID($order_no, $goods_barcode = '', $fields = '*') {
        $condition['order_no'] = array('eq', $order_no);
        if ($goods_barcode) {
            $condition['goods_barcode'] = array('eq', $goods_barcode);
        }
        $common_info = $this->table('scm_client_order')->field($fields)->where($condition)->find();
        return $common_info;
    }
     ...
     * @return Ambigous <multitype:boolean Ambigous <string, mixed> , unknown>
     */
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_no desc', $limit = '', $extend = array(), $master = false){
        $list = $this->table('scm_client_order')->field($field)->where($condition)->page($pagesize)->order($order)->limit($limit)->master($master)->select();
...
//        }
    ...
    public function getOrderGoodsList($condition = array(), $fields = '*', $limit = null, $page = null, $order = 'goods_barcode desc', $group = null, $key = null) {
        return $this->table('scm_order_goods')->field($fields)->where($condition)->limit($limit)->order($order)->group($group)->key($key)->page($page)->select();
    }
    ...
    public function editOrder($data, $condition = array(), $limit = '') {
        $update = $this->table('scm_client_order')->where($condition)->limit($limit)->update($data);
//        if ($update) {
//            //更新缓存
//            if (C('cache_open')) {
