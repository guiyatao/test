...
class scm_supp_clientModel extends Model{
     ...
     * @return array
     */
    public function getSuppList($condition = array(), $page = '', $limit = '', $fields = '*', $order = 'scm_supp_client.supp_id desc') {
        $on = 'scm_supp_client.supp_id = scm_supplier.supp_id';
        $result = $this->table('scm_supp_client,scm_supplier')->field($fields)->join('inner')->on($on)->where($condition)->page($page)->limit($limit)->order($order)->select();
        return $result;
    }
