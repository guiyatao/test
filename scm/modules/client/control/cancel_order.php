...
class cancel_orderControl extends SCMControl{
    /**
     * 每次导出订单数量
     * @var int
     */
    ...
    public function indexOp(){
        Tpl::showpage('cancel_order.index');
    }
    ...
    public function get_xmlOp(){
        ...
        $condition['pay_flag'] = array('eq', 1);  //已付款
        $condition['out_flag'] = array('eq', 0);  //未发货
...
//            $model_order->getOrderExtendInfo($order_info);
            ...
            $list['operation'] = "<a class='btn red' href='javascript:void(0);' onclick=\"fg_cancel_order('" . $order_info['order_no'] . "')\"><i class='fa fa-ban'></i>取消订单</a>";
...
//			$list['evaluation_state'] = str_replace(array(0,1,2), array('未评价','已评价','未评价'),$order_info['evaluation_state']);
            ...
            $list['prepare_flag'] = $order_info['prepare_flag'] ? '备货完成' : '未备货';
...
//       Tpl::output('store_info',$store_info);
    ...
    private function createExcel($data = array()){
        ...
        foreach ((array)$data as $k=>$order_info){
            ...
            $tmp[] = array('data'=> $order_info['prepare_flag'] ? '备货完成' : '未备货');
            $excel_data[] = $tmp;
        }
