...
class refund_not_inControl extends SCMControl{
    protected $user_info;
    ...
    public function listRefundGoodsOp() {
        ...
        if (chksubmit()){
            ...
            $state = $model_order->doRefundOrder($goods_array, $clie_id);
            if ($state) {
                showMessage(Language::get('nc_common_save_succ'),'index.php?act=refund_not_in&op=index');
            } else {
        ...
        $condition['refund_flag'] = array('neq', 1);
        $list = $model_order->getOrderList($condition);
        Tpl::output('list',$list);
        Tpl::showpage('refund_not_in.list');
    }
