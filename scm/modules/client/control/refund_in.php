...
class refund_inControl extends SCMControl{
    protected $user_info;
     ...
     *
     */
    public function createRefundNo($clie_id) {
        $refund_no = substr($clie_id, 0, 4) . '101' . time();
        return $refund_no;
    }
     ...
     * 缺货列表
     */
    public function listRefundGoodsOp() {
        $clie_id = $this->user_info['supp_clie_id'];
        $model_stock = SCMModel('scm_client_stock');
        if (chksubmit()){
            $goods_array = $_POST;
            unset($goods_array['form_submit']); 
            $refund_no = $this->createRefundNo($clie_id);
            $state = $model_stock->createRefundToSupp($goods_array, $refund_no, $clie_id);
            if ($state) {
               //减少终端店库存
                $where = array();
                $stock = array();
                ...
                foreach ($goods_array as $good => $num) {
                    $where['goods_barcode'] = $good;
                    $stock['set_num'] = $num;
                    $model_stock->editStock($stock, $where, '-');
                }
                showMessage(Language::get('nc_common_save_succ'),'index.php?act=refund_in&op=index');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
        $field = 'clie_id, goods_barcode, goods_nm,
            goods_price, goods_discount, (goods_price * goods_discount) goods_discount_price,
            goods_unit, goods_spec, goods_stock,
            goods_low_stock, supp_id';
        $list = $model_stock->getStockList(array('clie_id'=>$clie_id), '', $field);
        Tpl::output('list',$list);
        Tpl::showpage('refund_in.list');
    }
