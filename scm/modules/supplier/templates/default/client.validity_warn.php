...
<script>
    ...
    $(function(){
        $("#flexigrid").flexigrid({
            url:'index.php?act=client&op=get_validation_warn_xml&clie_id=<?=$output['clie_id']?>',
            colModel:[
                ...
                {display: '生产日期', name : 'production_date', width : 130, sortable : false, align: 'center'},
                {display: '有效期至', name : 'expire_date', width : 130, sortable : false, align: 'center'},
    ...
    function fg_csv(ids) {
        id = ids.join(',');
        window.location.href = 'index.php?act=client&op=export_validity_warn_csv&id=' + id +'&clie_id=<?=$output['clie_id']?>';
    }
