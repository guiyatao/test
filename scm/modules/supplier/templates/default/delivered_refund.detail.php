...
<!--                <th width="60" align="center">允许退货</th>-->
<!--                <th width="60" align="center">不允许退货</th>-->
                <th></th>
            </tr>
            </thead>
            <tbody>
            <?php if(!empty($output['refund_list']) && is_array($output['refund_list'])){ ?>
                <?php foreach($output['refund_list'] as $k => $v){ ?>
                    <tr>
                        ...
                        <td><?=$v['refund_date'] ?></td>
<!--                        <td><input type="radio" name="allow_refund_--><?//=$v['id'];?><!--[]" value="1" /></td>-->
<!--                        <td><input type="radio" name="allow_refund_--><?//=$v['id'];?><!--[]" value="2" /></td>-->
                        <td></td>
                    </tr>
                <?php } ?>
            <?php }else { ?>
...
<script>
    ...
    function fg_csv(ids) {
        id = ids.join(',');
        window.location.href = 'index.php?act=delivered_refund&op=export_detail_csv&refund_no=<?=$output['refund_no'] ?>&id=' + id;
    }
    function fg_del(ids) {
