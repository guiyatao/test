...
class all_delivered_refundControl extends SCMControl
{
    ...
    {
        Tpl::showpage('all_delivered_refund.index');
    }
    ...
    public function get_xmlOp()
    {
        ...
        foreach ($refund_list as $value) {
            $param = array();
            $index++;
            $param['operation'] = "<a class='btn blue' href='index.php?act=all_delivered_refund&op=detail&refund_no=" . $value['refund_no'] . "'><i class='fa fa-pencil-square-o'></i>查看详情</a>";
            $param['number'] = $index;
            ...
            /*
             * refund_flag 全部为0,已提交退货
             * refund_flag 部分为0,正在处理退货
             * refund_flag 全部为1,为退货完成
             */
   ...
   */
    public function detailOp(){
        ...
        Tpl::showpage('all_delivered_refund.detail');
    }
    ...
    private function createCsv($refund_list) {
        ...
        $header = array(
            ...
            "total" => iconv('utf-8','gb2312', "退款总额(元)"),
    ...
    private function createDetailCsv($refund_list) {
        ...
        foreach ($refund_list as $value) {
            ...
            if($value['refund_flag'] == 0)
                $param['refund_flag'] = iconv('utf-8','gb2312', '退货失败');
            else if($value['refund_flag'] == 1)
                $param['refund_flag'] = iconv('utf-8','gb2312', '退货成功');
            $param['refund_date'] = $value['refund_date'];
            $data[$value['id']] = $param;
        }
        $header = array(
