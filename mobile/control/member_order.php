...
//         $data['refund_state'] = $result['data']['order_info']['refund_state'];
        ...
        $data['pickup_type'] = $result['data']['order_info']['extend_order_common']['pickup_type'];
        $pickup_store_id = $result['data']['order_info']['extend_order_common']['pickup_store'];
        
        if($pickup_store_id) {
            $model_scm_client = SCMModel("scm_client");
            $clientinfo = $model_scm_client->getClientInfo(array("clie_id" => $pickup_store_id));
            $data['pickup_store']['clie_id'] = $clientinfo['clie_id'];
            $data['pickup_store']['name'] = $clientinfo['clie_ch_name'];
            $data['pickup_store']['address'] = $clientinfo['area_province'].$clientinfo['area_city'].$clientinfo['area_district'].$clientinfo['clie_address'];
        }
        
        output_data(array('order_info'=>$data));
    }

    public function aroundstoreOp() {
        
        $goodsids = $_POST['goodsids'];
        $address = $_POST['address'];

            $model_setting = SCMModel('shoponlineorder');
        $geo = $model_setting->getGeoByAddress($address);
        $ret = $model_setting->getStoreByGoodsid($goodsids, $geo);
        if(count($ret['client']) == 0) {
            $ret = $model_setting->getStoreByGoodsid($goodsids, $geo, 2);
        }
        $ret2 = array();
        if(count($ret) > 0) {
            foreach($ret['client'] as $k=>$v) {
                $row = array();
                $row['id'] = $k;
                $row['name'] = $v["name"];
                $row['address'] = $v["address"];
                $row['tel'] = $v['phone']["tel"];
                $row['mobile']  = $v['phone']['mobile'];
                $ret2[] = $row;
            }
        }
        ...
        if($ret['totalcnt'] == count(explode(",", $goodsids))) {
            $tojson['hasstore'] = 1;
            $tojson['result'] = $ret2;
        } else {
            $tojson['hasstore'] = 0;
