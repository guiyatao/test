 ...
 * @return obj 对象形式的返回结果
 */
function SCMModel($model = null){
    static $_cache = array();
    if (!is_null($model) && isset($_cache[$model])) return $_cache[$model];
    $file_name = BASE_SCM_PATH.'/model/'.$model.'.model.php';
    $class_name = $model.'Model';
    if (!file_exists($file_name)){
        return $_cache[$model] =  new Model($model);
    }else{
...
function urlSCM($act = '', $op = '', $args = array()){
    return url($act, $op, $args, false, SCM_SITE_URL);
}
function urlSCMGzkj($act = '', $op = '', $args = array()){
    return url($act, $op, $args, false, SCM_SITE_URL.DS.SCM_MODULES_GZKJ);
}
function urlSCMSupplier($act = '', $op = '', $args = array()){
    return url($act, $op, $args, false, SCM_SITE_URL.DS.SCM_MODULES_SUPPLIER);
}
function urlSCMClient($act = '', $op = '', $args = array()){
    return url($act, $op, $args, false, SCM_SITE_URL.DS.SCM_MODULES_CLIENT);
}
/**
