...
<div class="header">
  <div class="layout">
    ...
    <div class="version">版本: 2015.09.10.12636</div>
  </div>
</div>
...
function step3(&$install_error,&$install_recover){
    ...
    if (file_exists('data/'.$_charset.'_cms.sql')) {
        $sql .= "\r\n".file_get_contents("data/{$_charset}_cms.sql");
    }
    if (file_exists('data/'.$_charset.'_circle.sql')) {
        $sql .= "\r\n".file_get_contents("data/{$_charset}_circle.sql");
    }
    if (file_exists('data/'.$_charset.'_microshop.sql')) {
