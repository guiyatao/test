...
class Control{
     ...
     * wx
     */
    protected final function wxMsgSend($client, $msg, $touser_id = 'o1KiwwzsnIPMOUXIp0EdyUCpSn4k') {
        $appid = "wx36b98215337c13ff";
        ...
        $sql = "SELECT * FROM `gzkj`.`gzkj_scm_client_stock`
            WHERE(shelf_life LIKE '%月' AND datediff(DATE_ADD(production_date,INTERVAL (shelf_life * 30) DAY), NOW()) < valid_remind)
            OR (shelf_life LIKE '%天' AND datediff(DATE_ADD(production_date,INTERVAL shelf_life DAY), NOW()) < valid_remind);";

        $date_warn_list = $model_stock->getWarnGoodsList($sql);
        $date_warn_count = count($date_warn_list);

        $sql = "SELECT * FROM `gzkj`.`gzkj_scm_client_stock` WHERE goods_stock > goods_low_stock AND goods_barcode NOT IN
                (SELECT goods_barcode FROM `gzkj`.`gzkj_scm_client_order` WHERE order_date > DATE_SUB(NOW(),INTERVAL 30 DAY))";

        $unsalable_list = $model_stock->getWarnGoodsList($sql);
        $unsalable_count = count($unsalable_list);

        //$touser_id从user表中获取openid
        // $touser_id = 'o1KiwwzsnIPMOUXIp0EdyUCpSn4k';
        if ($access_token && ($stockout_count > 0 || $date_warn_count > 0 || $unsalable_count > 0)) {
            ...
            // foreach ($stockout_list as $value) {
                ...
                $info = array(
                        'touser' => $touser_id,
                        "template_id" => "q79VaJndOB3FBeo1Y-S3gQltiA-ZD3ORbi8WWYpN8rE",
                        "data" => array(
                            "first" => array(
                                    //"value" => "尊敬的".$stockout_list[0]['clie_id']."店主",
                                    "value" => "尊敬的".$client."店主",
                                    "color" => "#173177"
                                ),
                            "keyword1" => array(
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'statistics' => array('name' => '统计结算', 'child' => array(
                array('name' => '商城概况', 'act'=>'statistics_general', 'op'=>'general'),
