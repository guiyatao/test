...
//      Tpl::output('show_page',$page->show());
    ...
    public function album_pic_watermarkOp(){
        ...
        if (1) {
            $imager = \Shopnc\Lib::imager();

            if ($store_wm_info['wm_image_name']) {
                $wmLogoPath = BASE_UPLOAD_PATH.DS.ATTACH_WATERMARK.DS.$store_wm_info['wm_image_name'];
                $wmLogo = $imager->createImageFromPath($wmLogoPath);
            } else {
                $wmLogo = null;
            }

            if ($store_wm_info['wm_text']) {
                $wmFont = BASE_DATA_PATH.DS.'resource'.DS.'font'.DS.$store_wm_info['wm_text_font'].'.ttf';
                $wmText = $imager->createText(
                    $store_wm_info['wm_text'],
                    $store_wm_info['wm_text_color'],
                    0, // alpha 0~127
                    $store_wm_info['wm_text_size'],
                    5, // padding
                    $wmFont
                );
            } else {
            ...
            foreach ($wm_list as $v) {
                $imgPath = BASE_UPLOAD_PATH.DS.ATTACH_GOODS.DS.$_SESSION['store_id'].DS.
                    str_ireplace('.', '_1280.', $v['apic_cover']);

                $tmpImg = $imager->createImageFromPath($imgPath);

                if ($wmLogo) {
                    $tmpImg = $tmpImg->draw($wmLogo, $store_wm_info['wm_image_pos']);
                }
                if ($wmText) {
                    $tmpImg = $tmpImg->draw($wmText, $store_wm_info['wm_text_pos']);
                }
