...
class member_orderControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $pickuptype = $order_info['extend_order_common']['pickup_type'];
            $pickuptypemsg="";
            if($pickuptype==1) {
                $pickuptypemsg="自提";
            ...
            $order_info['pickup_type'] = $pickuptypemsg;
            //显示取消订单
    ...
    public function show_orderOp() {
        ...
        if($orderinfo && count($orderinfo) > 0) {
            ...
            $client = $model_scm_client->getClientInfo(array("clie_id"=>$clie_id));

            $client_address = $client['area_province'].$client['area_city'].$client['area_district'].$client['clie_address'];
            if($client['clie_tel'] && $client['clie_mobile']) {
                $client_phone = $client['clie_tel'].'/'.$client['clie_mobile'];
            } else if ($client['clie_mobile']) {
                $client_phone = $client['clie_mobile'];
            } else {
                $client_phone = $client['clie_tel'];
            }
            Tpl::output('client_address_phone', $client_address. " " . $client_phone);
        }
