 ...
 *
 *
 * @copyright  Copyright (c) 2007-2016 共铸商城  Inc. (http://www.gongzhuying.com)
...
class dateControl extends BaseCronControl {
    ...
    private function _testWechat() {
        ...
        $all_clients = $model_stock->getAllClients($condition);
        $alert_time = date("Y-m-d H:i:s",time());
        foreach ($all_clients as $client) {
            $condition = array();
            $condition['clie_id'] = $client['clie_id'];
            $stockout_list = $model_stock->getStockoutList($condition);
            $stockout_count = count($stockout_list);
            $sql = "SELECT * FROM `gzkj`.`gzkj_scm_instock_info`
                WHERE(clie_id = '{$client['clie_id']}' AND shelf_life LIKE '%月' AND datediff(DATE_ADD(production_date,INTERVAL (shelf_life * 30) DAY), NOW()) < valid_remind)
                OR (clie_id = '{$client['clie_id']}' AND shelf_life LIKE '%天' AND datediff(DATE_ADD(production_date,INTERVAL shelf_life DAY), NOW()) < valid_remind);";

            $date_warn_list = $model_stock->getWarnGoodsList($sql);
            $date_warn_count = count($date_warn_list);

            $sql = "SELECT * FROM `gzkj`.`gzkj_scm_client_stock` WHERE clie_id = '{$client['clie_id']}' AND goods_stock > goods_low_stock AND goods_barcode NOT IN
                    (SELECT goods_barcode FROM `gzkj`.`gzkj_scm_instock_info` WHERE clie_id = '{$client['clie_id']}' AND in_stock_date > DATE_SUB(NOW(),INTERVAL 30 DAY))";

            $unsalable_list = $model_stock->getWarnGoodsList($sql);
            ...
            foreach ((array)$update_arr as $k=>$v){
                foreach ($v as $m_k=>$m_v){
                    ...
                    if ($statmember_info){
                        //构造更新数组
                        $tmp_update = array();
                        $tmp_update['statm_ordernum'] = $orderlist_amount[0]['num'];
                        $tmp_update['statm_orderamount'] = floatval($orderlist_amount[0]['amount']);
                        $tmp_update['statm_goodsnum'] = floatval($ordergoods_amount[0]['gnum']);
                        $tmp_update['statm_updatetime'] = $stime;
                        $model->updateStatmember(array('statm_time'=>$k,'statm_memberid'=>$m_k),$tmp_update);
                    } else {
                        $tmp = array();
