...
class messageModel extends Model {
    ...
    public function listMessage($condition,$page='') {
        ...
        $message_list       = $this->select1($param,$page);
        return $message_list;
    }
    /**
     * 卖家站内信列表
    ...
    public function getRowMessage($condition) {
        ...
        $param['table']     = 'message';
        $param['where']     = $condition_str;
        $message_list       = $this->select1($param);
    ...
    public function saveMessage($param) {
        ...
        return $this->insert1('message',$array);
    }
    /**
     * 更新站内信
     */
    public function updateCommonMessage($param,$condition){
        ...
        //得到条件语句
        $condition_str = $this->getCondition($condition);
        $this->update1('message',$param,$condition_str);
    }
    /**
     * 删除发送信息
     */
    public function dropCommonMessage($condition,$drop_type){
        ...
        //查询站内信列表
        $message_list   = array();
        $message_list = $this->select1(array('table'=>'message','where'=>$condition_str,'field'=>'message_id,from_member_id,to_member_id,message_state,message_open'));
        unset($condition_str);
        ...
        if (!empty($delmessage_id)){
            ...
            $this->delete1('message',$condition_str);
            unset($condition_str);
        }
        if (!empty($updatemessage_id)){
            ...
            if ($drop_type == 'msg_private') {
                $this->update1('message',array('message_state'=>1),$condition_str);
            }elseif ($drop_type == 'msg_list') {
                $this->update1('message',array('message_state'=>2),$condition_str);
            }
        }
        return true;
    }
    ...
    public function dropBatchMessage($condition,$to_member_id){
        ...
        //查询站内信列表
        $message_list   = array();
        $message_list = $this->select1(array('table'=>'message','where'=>$condition_str));
        unset($condition_str);
        if (empty($message_list)){
            return true;
        }
        foreach ($message_list as $k=>$v){
            ...
            if ($tmp_delid_str == $v['to_member_id']){//所有用户已经全部阅读过了可以删除
                $this->delete1('message'," message_id = '{$v['message_id']}' ");
            }else {
                $this->update1('message',array('del_member_id'=>$tmp_delid_str)," message_id = '{$v['message_id']}' ");
            }
        }
        return true;
    }
