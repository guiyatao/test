...
class refund_returnModel extends Model{
    ...
    public function getPayDetailInfo($detail_array) {
        ...
        foreach ($order_list as $key => $order_info) {//在线支付金额
            if(in_array($order_info['payment_code'],array('wxpay','wx_jsapi','wx_saoma'))) $pay_amount += ncPriceFormat($order_info['order_amount']-$order_info['rcb_amount']-$order_info['pd_amount']);
        }
