...
class stat_tradeControl extends SystemControl{
    ...
    public function incomeOp(){
        ...
        if($_GET['exporttype'] == 'excel'){
            ...
            $bill_list = $model->getBillList($where, "*", 0, 0, 'ob_id desc');
            //导出Excel
...
//                $excel_data[$k+1][5] = array('data'=>$v['ob_store_cost_totals']);
    ...
    public function get_plat_incomeOp(){
        ...
        echo '<li title="退款金额：'. number_format($plat_data[0]['ob_order_return_totals'],2).'元"><h4>退款金额</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'.$plat_data[0]['ob_order_return_totals'].'"></h2><h6>元</h6></li>';
        echo '<li title="实收金额：'. (number_format($plat_data[0]['ob_order_totals']-$plat_data[0]['ob_order_return_totals'],2)).'元"><h4>实收金额</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'. ($plat_data[0]['ob_order_totals']-$plat_data[0]['ob_order_return_totals']).'"></h2><h6>元</h6></li>';
        echo '<li title="佣金总额：'. number_format($plat_data[0]['ob_commis_totals']-$plat_data[0]['ob_commis_return_totals'],2).'元"><h4>佣金总额</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'. ($plat_data[0]['ob_commis_totals']-$plat_data[0]['ob_commis_return_totals']).'"></h2><h6>元</h6></li>';
//        echo '<li title="店铺费用：'. number_format($plat_data[0]['ob_store_cost_totals'],2).'元"><h4>店铺费用</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'.$plat_data[0]['ob_store_cost_totals'].'"></h2><h6>元</h6></li>';
        echo '<li title="总收入：'. number_format($plat_data[0]['ob_result_totals'],2).'元"><h4>总收入</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'.$plat_data[0]['ob_result_totals'].'"></h2><h6>元</h6></li>';
		echo '</ul></dd><dl>';
        exit();
    }
    ...
    public function get_income_xmlOp(){
        ...
        //店铺数据
        //$field = array('min(ob_id) ob_id, min(ob_store_id) ob_store_id, min(ob_store_name) ob_store_name, SUM(ob_order_totals) as ob_order_totals','SUM(ob_commis_totals) as ob_commis_totals','SUM(ob_order_return_totals) as ob_order_return_totals','SUM(ob_commis_return_totals) as ob_commis_return_totals','SUM(ob_store_cost_totals) as ob_store_cost_totals','SUM(ob_result_totals) as ob_result_totals');
        $list = $model->getBillList($where, "*", $page, 0, $orderby);
        $statlist = array();
...
//                $out_array['ob_store_name'] = '<a href="'.urlShop('show_store', 'index', array('store_id' => $v['ob_store_id'])).'" target="_blank">'.$v['ob_store_name'].'</a>';
    ...
    public function get_sale_xmlOp(){
        ...
        $order_type = array('order_sn','buyer_name','order_add_time','order_amount','order_state');
        $sort_type = array('asc','desc');
        $sortname = trim($this->search_arr['sortname']);
        if (!in_array($sortname,$order_type)){
            $sortname = 'order_id';
